...
//mail list by json
var getJSON = async function (jsonurl, callback) {
...
//   }
// });
// getJSON("http://127.0.0.1:8000/api/files.json", function (err, data) {
//   if (err !== null) {
//     console.log("Something went wrong: " + err);
//   } else {
//     allFileList = data;
//     console.log('log', allFileList);
//      loadFileData(allFileList);
//   }
// });
let test = Date.now();
...
function loadFileData(datas) {
    ...
    Array.from(datas).forEach(function (fileData, index) {
        ...
        if (fileData.fileName.includes(".")) {
            ...
            var filetype = fileData.fileName.split(".").pop();
        ...
        <td>\
          <div class="d-flex gap-3 justify-content-center">\
            <button type="button" id="' + fileData.id + '" class="btn btn-ghost-primary btn-icon btn-sm favourite-btn ' + checkStarred + '">\
                ...
                fetch("/api/files/" + item.id, {
                    ...
                    .then(function (data) {
                        ...
                        if (data.status === 200) {
                            ...
                            'Erreur lors de la suppression du statut favoris';
                        }
...
Array.from(document.querySelectorAll('.file-manager-menu a')).forEach(function (fileTab) {
    fileTab.addEventListener("click", function () {
        ...
        // if (tabname === 'Media') {
        //   var filterData = allFileList.filter(filelist => filelist.category === 'Media');
        ...
        if (tabname != 'My Drive' && tabname != 'Important' && tabname != 'Recents') {
            console.log('ici')
            var filterData = allFileList.filter(filelist => filelist.category === tabname);
            document.getElementById("folder-list").style.display = "none";
    ...
    Array.from(document.querySelectorAll(".remove-list")).forEach(function (item) {
        item.addEventListener('click', function (event) {
            ...
            document.getElementById("remove-fileitem").addEventListener("click", function () {
                ...
                // const request = fetch("/api/files/" + getid, {
                //   method :'DELETE',
                // })
                //     .then(function (response) {
                //       return response;
                ...
                //       console.log(data);
                //       if (data.status === 204) {
                //         alert('Fichier correctement supprimé');
                //
                //         fetch("/file/" + getid, {
                //           method: 'POST'
                //         })
                ...
                //           return answer;
                //         })
                //             .then(function(deletionDatas) {
                //               console.log(deletionDatas);
                //               return deletionDatas;
                //             })
                //
                //       } else {
                ...
                const request = fetch("/file/" + getid, {
                    method: 'POST',
                })
                    .then(function (response) {
...
function fileDetailShow() {
    ...
    Array.from(document.querySelectorAll("#file-list tr")).forEach(function (item) {
        item.querySelector(".viewfile-list").addEventListener("click", function () {
            ...
            var filelistType = item.querySelector(".filelist-category").innerHTML;
